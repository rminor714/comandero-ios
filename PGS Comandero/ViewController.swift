...
class ViewController: UIViewController, WKNavigationDelegate {
    ...
    @objc func keyboardWillHide(_ notification: NSNotification) {
        PGSComandero.webView.setNeedsLayout()
    }
    
    func initWebView() {
        PGSComandero.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(PGSComandero.webView);
        PGSComandero.webView.uiDelegate = self;
        PGSComandero.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)
        // PGSComandero.webView.configuration.userContentController.add(self, name: "getRequest")
        
        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = PGSComandero.webView.observe(\.underPageBackgroundColor) { [unowned self] webView, _ in
                currentWebViewTheme = PGSComandero.webView.underPageBackgroundColor.isLight() ?? true ? .light : .dark
...
//        self.splashBkgView.frame = CGRect(x: splashBkgFrame.minX, y: statusBarHeight, width: splashBkgFrame.width, height: splashBkgFrame.height)
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !PGSComandero.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl() {
        ...
        print("ESTA ES LA URL: \(launchUrl)")
        PGSComandero.webView.load(URLRequest(url: rootUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            PGSComandero.webView.isHidden = false;
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                PGSComandero.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(PGSComandero.webView.estimatedProgress);
    ...
    deinit {
        PGSComandero.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
  ...
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
      print(message.name)
        if message.name == "print" {
            printView(webView: PGSComandero.webView)
        }
      ...
      if (message.name == "getRequest") {
          ...
          if let objStr = message.body as? String {
              ...
              do {
                  ...
                  print(jsObj)
                  if let config = jsObj as? Dictionary<String, Any> {
                      let url = URL(string: config["url"] as! String)
                      ...
                      let task = URLSession.shared.dataTask(with: request) { data, response, error in
                          if let data = data {
                              print("withresponse", response ?? "NO RESPONSE")
                              DispatchQueue.main.async {
                                  PGSComandero.webView
                                      .evaluateJavaScript(
                                        "javascript:setResponse('success', '\(data)', '\(String(describing: config["id"]))')",
                                        completionHandler: {
                                            (data, error) in
                                            if let data = data {
                                                print("Received: ", data)
                              ...
                              DispatchQueue.main.async {
                                  PGSComandero.webView
                                      .evaluateJavaScript(
                                        "javascript:setResponse('error', '\(error)', '\(String(describing: config["id"]))')",
                                        completionHandler: {
                                            (data, error) in
                                            if let data = data {
                                                print("Received: ", data)
                  ...
                  PGSComandero.webView
                      .evaluateJavaScript(
                        "javascript:setResponse('error', '\(nserror.description)')",
                        completionHandler: {
                            (data, error) in
                            if let data = data {
                                print("Received: ", data)
